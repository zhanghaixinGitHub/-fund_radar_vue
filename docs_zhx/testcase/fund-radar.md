...
<a id="positioning-acceptance"></a>

## TC-POS-01｜持仓预警与决策辅助目标验收（待实施、未执行）

依据：[当前定位与证据要求](../requirements/fund-radar.md#current-positioning)。本节为新目标的验收约定，不代表功能或测试已完成。既有预测专项的禁用规则继续约束其自身输出，不代表整个项目永远不能给出有依据的买入或减仓建议。

| 场景 | 预期结果 |
| --- | --- |
| 已确认事实与基金关联成立，涨跌模型未发布 | 可展示独立事实提醒及来源/日期/覆盖边界，不生成未发布概率；依赖模型的建议仍不可用 |
| 本人确认的配置规则与必要持仓输入完整 | 可复算触发条件并解释相应加仓、持有或减仓建议；其他用户不可读取该持仓、规则或结果 |
| 候选基金满足拟定买入条件 | 展示买入依据、反对证据、适用范围及失效条件；仅“没查到风险”不满足条件 |
| 只知道净值高于均值、基金经理变更或一条负面新闻 | 不直接判断虚高、必跌或卖出；缺少估值口径/基金关联/规则时明确说明 |
| 底层持仓过旧、仅部分披露或关键输入冲突/缺失 | 明确覆盖与滞后，停止依赖该输入的建议；不能把未知当作零暴露或低风险 |
| 关注基金但未确认个人持仓 | 允许基金级事实检查，不生成个人仓位、盈亏或操作金额结论；只要求该结论实际需要的输入 |
| 重复事件、普通概率波动、实质证据变化、恢复、撤销与过期 | 普通重复或波动不产生反向操作提醒；实质变化更新事件状态并说明前后差异，失效建议不继续显示为有效 |
| 查看任一建议及后续复盘 | 可还原事实、推断、规则、输入版本与撤销条件；事实可核验、规则可复算，不将解释流畅或证据等级当作预测准确率 |
| 宣称建议能改善投资结果 | 提供符合历史可得性、执行时点和费用的样本外/前向验证；对照收益、回撤、误报漏报、换手与错过机会代价，不仅展示方向命中率 |
| 接收或查看建议 | 不执行外部交易、不自动修改持仓/定投；浏览器仅访问 Java，个人结果不进入公共缓存 |

后续实施时使用明确标注的隔离样例验证规则和权限，实际效果另行记录。不得把本次 Markdown 检查计作业务、数据库或线上验证通过。
