...
> 日期：2026-08-24
> 说明（2026-09-09 校正）：本文包含验收用例与历次执行证据，不是当前功能清单。已有迁移和实际验证，最新检查结果见[检查报告](../reviews/project-status-2026-09-09.md)；旧空复选框不代表功能尚未实现，历史通过也不代表本次重测通过。数据库语句为 PostgreSQL 方言，执行前须核对当前结构与范围。

> 范围变化：固定六只重点基金已改为数据库启用市场范围，现行范围用例见 TC-25；登录及个人数据隔离见 TC-22。TC-12、TC-18 至 TC-20 保留早期单用户或六只阶段记录，旧命令与接口不作为当前验收入口。
...
> 历史阶段用例：本机单用户假设已被登录后按用户隔离取代，当前权限验收见 TC-22；下方保留当时证据。
...
> 历史阶段用例：六只范围及 `focused` 命令仅代表 2026-08-27 的验收；当前市场范围见 TC-25，不将历史数量替换为当前数量。
...
> 历史阶段用例：固定六只与 `FOCUSED_*` 接口已被市场动态范围及四类任务取代，当前入口见[同步中心用例](sync-center-tasks.md)。
...
> 历史阶段用例：六只基线、旧命令与该日执行结果保留为证据；当前自动计划的存在不代表 Beat/Worker 正在运行，须在实际验收时确认。
