...
> 当前说明校正于 2026-09-09：本文件保留总体方案与历次设计，不代表全部已实现。实际状态见[检查报告](../reviews/project-status-2026-09-09.md)和[总进度看板](../implementation/fund-radar.md)。登录隔离以第 12 节为准，基金同步范围以第 13 节为准；第 9 至 11 节的固定六只范围与旧同步入口仅作历史记录。

## 0. 当前产品目标与证据链（2026-09-09）

当前定位为**持仓预警与决策辅助**，以[总需求第 1 节](../requirements/fund-radar.md#current-positioning)为准。覆盖已持有和准备买入的基金，允许有依据的买入、加仓、持有、观察、暂缓与减仓建议。本节是目标设计，尚未实现，不新增接口、表、任务或数据源；后文的单基金概率契约继续作为辅助模块历史设计，不是整个产品的输出边界。

目标链路为：已授权事实 → 日期/质量/实体关联校验 → 事实、规则、模型分别判定 → Java 结合本人确认输入 → 带依据的建议 → 实质变化提醒 → 历史复盘。模型仅在被使用的分支执行准入检查，不是所有预警的统一前置条件。

### 0.1 建议的逻辑输出

以下为需求层的信息结构，正式字段名、API 与存储方式在对应实施任务中确定，不能称为现有接口：

| 内容 | 约束 |
| --- | --- |
| 对象与范围 | 区分候选基金、已持有基金和个人组合；关注关系不能证明实际持仓 |
| 事实与关联 | 保存可追溯来源、报告期、公开时间、截至日、关联对象与方法；底层暴露须有已披露占比、日期和覆盖限制 |
| 推断与规则 | 标明事实、推断、用户确认规则，记录使用的证据及版本；证据强度、风险等级不能伪装为下跌概率 |
| 建议与解释 | 明确买入/加仓/持有/观察/暂缓/减仓及其原因、反对证据、所需个人输入和适用范围；不足时只返回可证实内容与缺口 |
| 生命周期 | 记录触发、更新、恢复、撤销、过期及原因；同一事件关联前后版本，普通概率波动不直接切换操作建议 |
| 验证信息 | 事实可追溯、规则可复算；使用模型时关联模型和验证版本。若宣称改善投资结果，另关联含费用与执行时点的效果验证 |

### 0.2 分工与失败处理

- Python 延续非个人化的数据、事件、基金关联和模型职责；Java 延续认证、资源授权、本人持仓/规则、建议编排与提醒审计；Vue 展示依据与变化原因。共享基金缓存不混入个人建议，Python 不接收个人持仓。
- 净值偏高、超过历史均值不能直接推导高估；经理变化或一条负面新闻不能直接推导卖出。买入也必须具有正向依据和适用条件，不能由“未发现风险”生成。
- 事实提醒按来源、时效和关联验收；用户规则按必要输入及可复算性验收；模型分支按对应验证与发布门槛验收。缺哪个关键输入，就停止依赖它的建议，不补造概率或个人信息。
- 提醒仅在新证据或已确认条件有实质变化时投递；旧信息失效时更新原提醒而非继续展示为有效建议。去重、冷却和上限须按规则定义；历史记录支持复盘误报、漏报、费用及错过机会的代价。
- 当前只定义目标，不恢复已暂停资讯采集、不调用外部交易接口、不改变持仓或定投；后续新增来源仍按真实授权范围接入。
...
早期接口字段、分页和错误码见[接口契约](fund-radar-api-v1.md)，后续扩展另见对应专项设计。M1 已接入登记的 Tushare 基金并落库真实净值与完整资料；2026-09-09 检查时为 43 只基金。同步范围由数据库中的 `ACTIVE + TUSHARE_PRO_FUND` 动态确定，个人关注不参与选取；真正全市场目录完整性仍未完成，不再使用六只手工样本作为当前范围。
...
| P1 | 账号与个人数据管理 | 注册、角色权限和按用户隔离已实现，见第 12 节；账号注销及完整删除/导出仍须单独完成，不等于已支持多租户 |
...
> 历史设计：固定六只基金的配置与命令已被第 13 节动态市场范围取代，不作为当前同步入口。
...
> 历史设计：此节 `FOCUSED_*` 类型、六只配置及旧路由已被第 13 节和[同步中心设计](sync-center-tasks.md)取代；保留当时的设计依据。
...
> 历史设计范围：本节保留当时 M3 单基金概率契约和准入流程。当前产品目标、建议范围及事实/规则/模型分支以第 0 节为准，不把模型门槛套用于独立事实或用户规则提醒。

本节描述当时 M2/M3/M4 的目标设计，不新增表、接口、任务或外部调用，也不表示模型能力已经可用。当时 M3 的用户入口为“我的关注 → 单基金详情 → 多周期预测”，输出基金层面的未来约 7/15/30/60 天方向概率、依据和数据缺口；详细设计见 `docs_zhx/design/watchlist-prediction-module.md`。它不改变前端仅调用 Java、Java 负责认证与审计、FastAPI 仅提供内部数据/模型服务以及交易执行不存在的架构边界。
