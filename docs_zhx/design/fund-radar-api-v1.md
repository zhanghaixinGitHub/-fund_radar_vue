...
> 适用范围校正（2026-09-09）：本文保留早期接口约定和示例，未覆盖全部后续接口，不是当前 API 全量清单。正式登录、角色权限和本人数据隔离已经落地；同步、完整资料和预测读取分别见对应专项设计。示例中的数量不是实时统计，“M0 本地单用户”只描述初始阶段；最新能力与代码入口见[检查报告](../reviews/project-status-2026-09-09.md)。本次不修改实际接口契约。
...
### `GET /api/v1/watchlist`（当前登录用户）

需要 `WATCHLIST_SELF_READ`，数据范围由 Java 认证上下文限定为本人关注。可选 `keyword` 最多 50 字符，去除首尾空白后按基金代码或名称包含匹配，名称不区分英文大小写；空白或不传时查询全部。可与 `fundType` 共同筛选，`page` 默认 1（1–10,000），`pageSize` 默认 10（1–50）。

名称沿用已有基金摘要，服务端每批最多读取 50 条本人关注并匹配，再计算 `totalCount`、`totalPages` 和当前页，保持原类型及关注时间排序；不局限于当前页，不新增名称快照或数据库迁移。搜索时摘要服务调用失败返回 `503/AI_SERVICE_UNAVAILABLE`，避免把不完整匹配显示为“无结果”；没有关键词时保留现有行情降级。`quota` 始终是本人总关注额度，不随搜索结果减少。

关注页沿用基金市场搜索表单，回车或点击“查询基金”从第一页查询；清空后再次查询恢复全部。关键词、类型和分页保存在 URL 中，从详情返回或浏览器返回时恢复。
