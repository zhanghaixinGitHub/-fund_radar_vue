...
- [x] 2.1 Python 草案统计：滚动 63 日窗口 DD / R / L20 分布、修复天数、波动率同类分位；< 500 交易日、货币基金、QDII 返回明确「不适用/数据不足」状态；按基金+净值水位短时效缓存（2026-09-17，`tests/test_portfolio_advice_draft_stats.py` 15 项通过）
- [x] 2.2 Python 路由：`GET /internal/v1/portfolio-advice/{fundCode}/draft-stats`，只读、无用户身份（2026-09-17）
- [x] 2.3 Python 测试：分位复算一致性、历史不足与特殊类别分支、缓存不携带用户身份（2026-09-17，相关回归 53 项通过；真实服务实测 006730/001632/008888 均 200）
- [x] 2.4 Java Flyway V17：创建 `holding_rule_draft`、`holding_rule_profile`（含唯一 ACTIVE 部分索引、不可改写触发器、中文列注释）（2026-09-17，`V17__create_holding_rule_tables.sql`，本地 fund_core 应用成功）
- [x] 2.5 Java 规则接口：草案查询 / 重新生成 / 规则确认 / 撤销四个端点，确认幂等、撤销留痕、微调限 ±20% 记 CUSTOM（2026-09-17）
- [x] 2.6 Java 调度接入：锁批次内增加「草案刷新」（仅统计指纹变化时），未确认草案刷新不影响已确认规则（2026-09-17）
- [x] 2.7 Java 测试：确认/撤销生命周期、重复确认不生成新版本、统计未变不重复生成草案、跨用户拒绝（2026-09-17，全量 131 项测试 0 失败）
- [x] 2.8 前端：「规则草案确认」区块——三档卡片含触发统计、默认不选中、微调仅步进、确认文案含「本人确认后生效，可随时撤销」（2026-09-17，type-check/lint/build 通过，契约测试新增 7 项、全量 87/87）
- [x] 2.9 阶段验收：真实浏览器走通草案查看→确认→撤销全流程；规则未确认时页面显示「尚未确认规则」；结果记入「实施验收记录」（2026-09-17 调度批次真实执行：23 只持仓基金草案各一份、零失败，`holding_rule_profile` 保持 0 条无任何自动确认；浏览器走通待用户登录确认）
...
| 阶段 2 规则草案与确认 | 🔶 2.1–2.9 完成，浏览器走通待用户确认 | — |
| 阶段 3 建议 V2 升级 | ⬜ 未开始 | — |
| 阶段 4 回看扩展 | ⬜ 未开始 | — |
...
### 2026-09-17 阶段 2 实施验收（步骤 2.1–2.9 完成，浏览器走通待确认）

- **Python 侧**：`get_draft_stats` 与 `GET /internal/v1/portfolio-advice/{fund_code}/draft-stats` 落地；口径（滚动 63 日窗口 DD/R/L20、修复天数、波动率同类截尾 253 点分位、分位线性插值、分红不复权）固化在 docstring；缓存键 = 基金 + 最新净值日，进程内上限 256 条、无用户身份。新增测试 15 项全绿，相关回归 53 项通过；真实服务实测 006730（保守档减仓线 −7.10%/止盈 +6.61%，触发 843/511 次）、001632、008888 均 200 且结构完整，缓存命中后响应 0.557s → 0.026s。
- **Java 侧**：Flyway `V17__create_holding_rule_tables.sql` 已应用；草案/规则五个端点（查询、重新生成、确认、撤销 + 历史）落地；微调限草案值 ±20%（含边界），超出 400；确认幂等、supersede/撤销留痕写 audit_log；调度批次内草案刷新靠统计指纹幂等。全量 `mvn test` 131 项 0 失败。
- **前端**：「规则草案」区块（三档卡片 + 触发统计 + 步进微调 + 二次确认撤销 + 「统计已更新」徽标 + 份额门槛降级）；契约测试新增 7 项、全量 87/87；type-check/lint/build 通过。
- **真实批次验收**：2026-09-17 22:28 重启后调度批次执行，`checked=23, failures=0`；库内 23 只持仓基金各归档一份草案（1 位用户），`holding_rule_profile` 保持 0 条——**未发生任何自动确认**，规则纪律成立。
- **已知限制**：① 同类波动率分位用截尾 253 点口径与本基金全历史口径分列（docstring 注明）；② 缓存键不含同类样本水位，样本增减时分位可能滞后一个水位周期；③ censoredCount 合并「前向窗口不足」与「未修复」两种含义；④ 微调以 4 位小数存储；⑤ 前端「统计已更新」以 sourceDraftId 不等判定（语义等价于统计截止日更新）；⑥ 浏览器走通待用户登录确认。
- **联调事件**：重启 Java 时旧进程未随 Maven 壳退出导致 8080 端口占用，已手工清理后正常启动；与本次功能无关。
