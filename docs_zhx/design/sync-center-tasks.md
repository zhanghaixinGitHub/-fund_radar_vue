...
`LocalSyncJobManager` 使用 `SyncJobSnapshot` 保存当前 Python 进程内的实时状态。`MARKET_NAV_INCREMENTAL` 按基金逐只更新进度；`MARKET_DETAIL` 的总步数为“基础资料、扩展净值及三类逐基金资料”，经理、规模、分红每读取一只基金就更新一次。
...
`LocalSyncJobManager` 在进程内限制一个活动任务，`TushareFundSyncService` 使用既有 PostgreSQL 咨询锁处理跨进程冲突。启动冲突返回 `409`，不排队、不重试；外部 Tushare 可恢复错误沿用既有受控失败逻辑。进度回调异常只记录警告，不能中断真实写库。
